// 파일명: src/components/ApplicationList.tsx
'use client';
...
// 조인된 데이터를 위한 인터페이스
interface JoinedApplication extends ApplicationRow {
  tester?: Pick<ProfileRow, 'by_id'> | null;
  applicant?: Pick<ProfileRow, 'by_id'> | null;
}
...
export default function ApplicationList() {
  ...
  const [modalData, setModalData] = useState<{
    isOpen: boolean;
    app: JoinedApplication | null;
    status: string;
    feedback: string;
  }>({ isOpen: false, app: null, status: '', feedback: '' });
  ...
  const fetchApplications = useCallback(async () => {
    let query = filterVisibleTestData(supabase
      ...
      .select(`
        *,
        tester:tester_id(by_id),
        applicant:user_id(by_id)
      `)); // 🚨 조인 구문 명확화
    ...
    if (error) console.error("신청서 로드 에러:", error);
    else setApplications(data as unknown as JoinedApplication[]);
  }, [activeTab]);
    ...
    try {
      ...
      if (user) {
        ...
        if (profile) {
          ...
          if (hasPermission(normalizeRole(profile.role), 'member.approve')) {
  ...
  useEffect(() => {
    startTransition(() => { void checkAuthAndFetch(); });
  }, [checkAuthAndFetch]);

  const handleClaimTest = async (appId: number) => {
    if (!window.confirm('이 신청자의 가입 테스트를 담당하시겠습니까?')) return;
    try {
      setProcessingId(appId);
      const { error } = await supabase.from('applications').update({ tester_id: myProfile?.id }).eq('id', appId);
  ...
  const openProcessModal = (app: JoinedApplication, status: string) => {
    setModalData({
      isOpen: true, app, status,
  ...
  const confirmProcess = async () => {
    ...
    try {
      ...
      const { error: appError } = await supabase
        .from('applications')
        .update({ status, test_result: finalTestResult })
        .eq('id', app.id);
      if (appError) throw appError;

      if (isPass && app.user_id) {
        const { error: pErr } = await supabase.from('profiles').update({ role: 'rookie', rookie_since: new Date().toISOString() }).eq('id', app.user_id);
        if (pErr) throw pErr;
      }
  ...
  if (!myProfile || !hasPermission(normalizeRole(myProfile.role), 'member.approve')) {
          ...
          <div key={app.id} className={`rounded-2xl border shadow-xl overflow-hidden transition-colors ${activeTab === 'completed' ? 'bg-gray-900/50 border-gray-800' : 'bg-gray-800 border-gray-700 hover:border-gray-600'}`}>
            ...
            <div className={`p-4 border-b flex justify-between items-center ${activeTab === 'completed' ? 'bg-gray-900 border-gray-800' : 'bg-gray-900/50 border-gray-700'}`}>
              <div className="flex items-center gap-3">
                <span className={`px-2.5 py-1 text-xs font-bold rounded-md ${app.race?.includes('Terran') ? 'bg-blue-900/50 text-blue-300' :
                    app.race?.includes('Protoss') ? 'bg-yellow-900/50 text-yellow-300' :
                      app.race?.includes('Zerg') ? 'bg-purple-900/50 text-purple-300' :
            ...
            <div className={`p-6 ${activeTab === 'completed' ? 'opacity-80' : ''}`}>
              <div className="flex items-end gap-3 mb-4">
                <h3 className="text-2xl font-black text-white">{app.user_id}</h3>
                {/* 디스코드 이름 표시 (있는 경우) */}
                {app.applicant && (
          ...
          <div className="bg-gray-800 w-full max-w-md p-6 rounded-2xl border border-gray-700 shadow-2xl animate-fade-in-down">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className={`flex-1 py-3 font-bold rounded-xl text-white transition-colors shadow-lg ${modalData.status === '합격' ? 'bg-emerald-600 hover:bg-emerald-500' : 'bg-red-600 hover:bg-red-500'
                  }`}
              >
