...
function buildTeams(players, perTeam, sortOption) {
  ...
  if (sortOption === 'balance') {
    const sorted = [...pool].sort((a, b) => (b.Ladder_MMR || 1000) - (a.Ladder_MMR || 1000));
    const teamA = [], teamB = [];
  ...
  if (sortOption === 'bottom') {
    const sorted = [...pool].sort((a, b) => (a.Ladder_MMR || 1000) - (b.Ladder_MMR || 1000));
    return { teamA: sorted.slice(0, perTeam), teamB: sorted.slice(perTeam) };
  }
                ...
                <div key={team} className={`p-3 rounded-xl border ${team === 'A' ? 'border-blue-800 bg-blue-950/20' : 'border-red-800 bg-red-950/20'}`}>
                  ...
                  {(team === 'A' ? proposal.teamA : proposal.teamB).map(p => (
                    <div key={p.id} className="text-xs text-gray-300 truncate flex items-center gap-1 mb-1">
                      ...
                      <span className="text-yellow-500 text-[10px]">{p.Ladder_MMR || 1000}점</span>
                    </div>
                  ))}
...
function TwoMatchSuggestion({ players, onSelectMatch }) {
  ...
  const perTeam = 3;
  const sorted = [...players].sort((a, b) => (b.Ladder_MMR || 1000) - (a.Ladder_MMR || 1000));
  ...
  const calcAvg = (team) => (team && team.length > 0) ? team.reduce((s, p) => s + (p.Ladder_MMR || 1000), 0) / team.length : 0;
...
export default function LadderDashboard({ onMatchEnter }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const { data: queue } = await filterVisibleTestAccounts(supabase
        .from('profiles')
        .select('id, ByID, discord_name, race, Ladder_MMR, role, is_in_queue, queue_joined_at')
        .eq('is_in_queue', true)
      ...
      if (myProposal) {
        const resolveProf = (id) => ({
          ...
          Ladder_MMR: profMap[id]?.Ladder_MMR || 1000,
        });
        const teamA = (myProposal.team_a_ids || []).map(resolveProf);
        const teamB = (myProposal.team_b_ids || []).map(resolveProf);
        const avgA = teamA.length ? teamA.reduce((s, p) => s + (p.Ladder_MMR || 1000), 0) / teamA.length : 1000;
        const avgB = teamB.length ? teamB.reduce((s, p) => s + (p.Ladder_MMR || 1000), 0) / teamB.length : 1000;
  ...
  const handleProposeMatch = async (typeStr, teamA, teamB) => {
    ...
    const avgA = teamA.reduce((s, p) => s + (p.Ladder_MMR || 1000), 0) / teamA.length;
    const avgB = teamB.reduce((s, p) => s + (p.Ladder_MMR || 1000), 0) / teamB.length;
  ...
  /** A팀 평균 MMR */
  const avgA = teams ? teams.teamA.reduce((s, p) => s + (p.Ladder_MMR || 1000), 0) / teams.teamA.length : 0;
  /** B팀 평균 MMR */
  const avgB = teams ? teams.teamB.reduce((s, p) => s + (p.Ladder_MMR || 1000), 0) / teams.teamB.length : 0;
          ...
          <div className="flex flex-wrap gap-6">
            ...
            <div>
              ...
              <p className="text-yellow-400 font-bold text-xl drop-shadow-[0_0_5px_rgba(234,179,8,0.6)]">
                {myProfile?.Ladder_MMR ?? myStats?.points ?? 1000}점
              </p>
            </div>
            <div>
              <p className="text-gray-500 text-[10px] uppercase tracking-wider">티어</p>
              <p className={`font-bold ${TIER_COLORS[getTier(myProfile?.Ladder_MMR ?? 1000)]}`}>
                {getTier(myProfile?.Ladder_MMR ?? 1000)}
              </p>
            </div>
        ...
        {queuePlayers.length === 0 ? (
          ...
          <div className="divide-y divide-cyan-900/20">
            {queuePlayers.map((p, idx) => {
              const tier = getTier(p.Ladder_MMR || 1000);
              ...
              return (
                <div key={p.id} className={`px-5 py-3 flex items-center justify-between hover:bg-cyan-900/10 transition-colors ${isMe ? 'bg-cyan-950/20' : ''}`}>
                  ...
                  <div className="flex items-center gap-4 text-xs">
                    ...
                    <span className="text-yellow-400 font-bold">{p.Ladder_MMR || 1000}점</span>
                ...
                <div className="space-y-1">
                  {teams.teamA.map(p => (
                    <div key={p.id} className="text-xs text-gray-300 flex items-center gap-2">
                      ...
                      <span className="text-yellow-400">{p.Ladder_MMR || 1000}점</span>
                    </div>
                  ))}
                </div>
              </div>
