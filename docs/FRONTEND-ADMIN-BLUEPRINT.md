...
- 가입 심사: `applications`
- 래더 보드: `ladder_rankings`
- 경기 기록: `ladder_record`
- 세트 상세: `ladder_match_sets`
- 베팅: `match_bets`
- 일반 게시판: `posts`
- 공지/운영 게시판: `notice_posts`, `admin_posts`
- 유저 알림: `notifications`
- 포인트 이력: `clanpoint_logs`
...
- 핵심 데이터: `profiles`, `notice_posts`, `notifications`, `ladder_rankings`, `ladder_record`
...
- 핵심 데이터: `ladder_rankings`, `profiles`
- 핵심 액션: 정렬, 내 순위 강조, 특정 유저 프로필 이동

- 매치 센터

- 목적: 현재 모집중/진행중 경기, 팀 현황, 세트 현황, 베팅 상태 표시
- 핵심 데이터: `ladder_record`, `ladder_match_sets`, `match_bets`, `profiles`
- 핵심 액션: 경기 참가, 경기 관전, 베팅, 세트 상태 확인

- 경기 기록

- 목적: 종료 경기 히스토리와 세트 결과 열람
- 핵심 데이터: `ladder_record`, `ladder_match_sets`, `profiles`
...
- 위젯 제안:
  ...
  - 현재 진행중 경기 수
  - 최근 관리자 게시글
- 핵심 데이터: `applications`, `profiles`, `notifications`, `ladder_record`, `admin_posts`
...
- 하위 화면:
  ...
  - 역할 변경/수습 승급 모달
  - 마스터 위임 재인증 플로우
- 핵심 데이터: `profiles`, `notifications`, `clanpoint_logs`
...
- 하위 화면:
  - 알림 발송 이력 및 수동 발송
  - 포인트 로그 조회
- 핵심 데이터: `notifications`, `clanpoint_logs`, `profiles`
...
- `ladder_record`는 유저 개인 기준의 기록으로 변경이 필요하며, 화면 렌더링 시 매핑이 필요합니다.
- `ladder_match_sets.team_a_entry`, `team_b_entry`는 JSONB라 화면마다 shape 보정이 필요합니다.
...
1. `clanpoint_logs`에 `source_type`, `source_id`, `balance_after` 추가
...
1. `ladder_record` 테이블의 구조를 개인 기준으로 명확히 변경 및 정규화

- 현재 구조는 빠르지만, 팀별 상태 관리와 감사 로그가 어려워집니다.

1. `ladder_match_sets` 엔트리 JSONB를 명시적 타입으로 고정
