...
import { PROJECT_INFO, CURRENT_STATUS } from '../../scripts/mobile/mobile-analyzer.js';
...
A: 개발 서버 테스트 단계, 홈페이지 전면 개선 완료

Q: 어떤 기능이 작동하나?
A: 기본 라우팅, 권한 시스템, 실시간 데이터 연동, 로딩/에러 상태, 홈페이지 4개 섹션

Q: 현재 문제점은?
A: 보안(환경변수 분리 필요), 권한 시스템 명확화 필요
...
- Supabase: 공개 키 사용 중
- 권한: 개발자가 모든 권한 보유 // 개발 + 클랜 운영을 위한 권한 설정(하지만 개발자가 마스터는 아님)
