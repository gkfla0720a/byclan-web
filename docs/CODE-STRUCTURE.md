...
4. [레이아웃 계층](#4-레이아웃-계층)
5. [컴포넌트 목록](#5-컴포넌트-목록)
6. [views 컴포넌트 목록](#6-views-컴포넌트-목록)
7. [훅 (Hooks)](#7-훅-hooks)
...
한국어 뷰 이름을 URL 경로로 변환하는 훅(`src/hooks/useNavigate.js`)에서 관리됩니다.
...
`src/views/` 는 Next.js 라우트 파일이 아닌, views 콘텐츠를 담당하는 컴포넌트 파일들입니다.

| 파일 | 설명 |
|------|------|
| `HomeContent.js` | 홈 화면 콘텐츠 (매치 현황, 활동 로그 등) |
| `ClanOverview.js` | 클랜 개요 views 콘텐츠 |
| `ClanMembers.js` | 클랜원 목록 views 콘텐츠 |
| `ClanTournament.js` | 토너먼트 views 콘텐츠 |
| `MediaGallery.js` | 미디어 갤러리 콘텐츠 |
| `PagePlaceholder.js` | 준비 중인 views용 플레이스홀더 |
...
navigateTo('BY래더); // → router.push('/ladder')
navigateTo('클랜원 관리'); // → router.push('/admin/guild')
```
...
```js
import { shouldBypassDiscordForTestAccount } from '@/utils/testData';
...
모바일 최적화 관련 내용은 `guides/MOBILE-GUIDE.md`를 참고하세요.
...
자세한 내용은 `guides/ENVIRONMENT-SETUP.md`를 참고하세요.
...
루트는 실행 설정 중심으로 유지하고, 운영 파일은 `scripts/`, `sql/`, `docs/` 아래로 분류했습니다.

| 파일 | 용도 |
|------|------|
| `scripts/seed/seed-test-data.cjs` | 테스트 데이터 Supabase에 주입 (`npm run seed:test-data`) |
| `scripts/seed/seed-test-accounts.js` | 테스트용 Auth/프로필 계정 생성 및 리셋 |
| `scripts/seed/seed-real-users.js` | 실사용자 샘플 시드 주입 |
| `scripts/verification/simulate-workflow-and-mmr.js` | 매치/세트/MMR/정산 E2E 시뮬레이션 검증 |
| `scripts/mobile/mobile-analyzer.js` | 모바일 반응형 분석 |
| `scripts/mobile/mobile-docs-updater.js` | 모바일 문서 자동 업데이트 |
| `sql/migrations/CLAN-POINT-SYSTEM-MIGRATION.sql` | 포인트/배팅/관리 감사 로그 스키마 및 트리거 |
| `sql/migrations/GLOBAL-ACTIVITY-LOG-MIGRATION.sql` | 전역 활동 로그(경기/CP/MMR/접속/게시글/닉네임/등급) 트리거 |
| `sql/migrations/MMR-SCORING-SYSTEM.sql` | 개인/팀/합산 MMR 정산 트리거 |
| `sql/policies/MATCH-BETS-RLS.sql` | match_bets 테이블 RLS 정책 |
| `sql/advisors/ADVISOR-WARNING-FIXES.sql` | Advisor 경고 보정 SQL |
...
| `docs/guides/ENVIRONMENT-SETUP.md` | 환경 변수 설정 및 개발 환경 구성 |
| `docs/guides/DATABASE-GUIDE.md` | Supabase SQL Editor 사용법 및 쿼리 참고 |
| `docs/guides/AUTH-TEST-ACCOUNT-STRATEGY.md` | 테스트 계정 생성/관리 전략 |
| `docs/guides/MOBILE-GUIDE.md` | 모바일 최적화 가이드 |
| `docs/worklog/WORKLOG_NEXT_STEPS.txt` | 개발 작업 로그 및 다음 작업 목록 |
| `sql/queries/DATABASE-QUERIES.sql` | 개발용 SQL 쿼리 모음 |
| `sql/migrations/GLOBAL-ACTIVITY-LOG-MIGRATION.sql` | 전역 활동 로그 DB 마이그레이션 (수동 변경 특수표시 포함) |
| `sql/policies/MATCH-BETS-RLS.sql` | match_bets 테이블 RLS 정책 SQL |
| `sql/migrations/PROFILE-DATA-BACKFILL.sql` | profiles 데이터 마이그레이션 SQL |
| `sql/migrations/STREAMER-FIELDS-MIGRATION.sql` | 스트리머 필드 마이그레이션 SQL |
| `sql/migrations/TEST-DATA-SEED.sql` | 테스트 데이터 SQL (Supabase SQL Editor용) |
