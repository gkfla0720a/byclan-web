...
현재 기본 로그인 UX는 일반 아이디 기반이며, 내부적으로는 `login.<아이디>@auth.byclan.local` 형식의 숨김 이메일을 사용합니다.
...
현재 서비스의 일반 아이디 로그인은 내부적으로 숨김 이메일을 사용합니다. 따라서 테스트 계정도 공개용 메일 주소 대신 `login.test1@auth.byclan.local` 같은 내부 이메일 형식으로 맞추는 편이 현재 앱 구조와 일치합니다.
