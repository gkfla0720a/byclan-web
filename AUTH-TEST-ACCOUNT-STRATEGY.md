...
현재 서비스는 이메일 인증 기반 회원가입을 사용하므로, 실제 로그인 검증용 테스트 계정은 인증 메일을 받을 수 있는 주소여야 합니다. 따라서 `test1@byclan.local` 같은 수신 불가능한 로컬 도메인 주소는 가입 완료 또는 로그인 검증 흐름에서 사용할 수 없습니다.
...
- 실제 로그인 검증: 수신 가능한 테스트 이메일 또는 Admin API로 미리 인증 완료 처리한 전용 계정 사용
